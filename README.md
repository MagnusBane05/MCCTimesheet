...
1. Run `npm run build:<environment>` (dev/prod) in a non-VSCode terminal (check `echo "$VITE_API_BASE_URL"` returns nothing. If not, run `unset VITE_API_BASE_URL` and `unset VITE_API_MODE`)
